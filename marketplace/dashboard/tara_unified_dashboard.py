...
with tab7:
    ...
    if product_data:
        ...
        # Ensure buy_now column exists
        if 'buy_now' not in product_df.columns:
            product_df['buy_now'] = "N/A"

        # Render clickable links for buy_now
        def render_buy_now_link(url):
            if url and url != "N/A":
                return f'<a href="{url}" target="_blank">Buy Now</a>'
            else:
                return "N/A"

        product_df['Buy Now'] = product_df['buy_now'].apply(render_buy_now_link)

        # Show dataframe except original buy_now column, replace with clickable 'Buy Now'
        display_cols = ["name", "cost", "price", "margin", "profit_pct", "stock", "Buy Now"]
        st.write(product_df[display_cols].to_html(escape=False, index=False), unsafe_allow_html=True)

        # Provide CSV download including original buy_now URLs
        csv = product_df.drop(columns=['Buy Now']).to_csv(index=False).encode('utf-8')
